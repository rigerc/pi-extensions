...
const MANAGED_ENV = [
  ...
  'OPENROUTER_API_KEY',
  SYSTEM_ONE_ENV.layaApiKey,
  SYSTEM_ONE_ENV.provider,
  SYSTEM_ONE_ENV.apiKey,
  SYSTEM_ONE_ENV.baseURL,
  SYSTEM_ONE_ENV.model,
  SYSTEM_ONE_ENV.secretsDir,
  LEGACY_JEV_ENV.provider,
  LEGACY_JEV_ENV.apiKey,
  LEGACY_JEV_ENV.baseURL,
  LEGACY_JEV_ENV.model,
  LEGACY_JEV_ENV.secretsDir,
] as const;
...
function isolateEnv(vars: Record<string, string> = {}) {
  ...
  const secretsDir = fs.mkdtempSync(path.join(os.tmpdir(), 'pi-system-one-secrets-'));
  process.env[SYSTEM_ONE_ENV.secretsDir] = secretsDir;
...
test('test_provider_explicit_laya_is_configured_without_an_api_key', () => {
  const env = isolateEnv({ [SYSTEM_ONE_ENV.provider]: 'laya' });
  try {
...
test('test_provider_laya_uses_specific_key_then_secret_file', () => {
  const fromEnv = isolateEnv({
    [SYSTEM_ONE_ENV.provider]: 'laya',
    [SYSTEM_ONE_ENV.layaApiKey]: 'laya-env-key',
  });
  try {
  ...
  const fromFile = isolateEnv({ [SYSTEM_ONE_ENV.provider]: 'laya' });
...
test('test_legacy_secrets_directory_is_accepted', () => {
  const env = isolateEnv({ [LEGACY_JEV_ENV.provider]: 'typesafe' });
  delete process.env[SYSTEM_ONE_ENV.secretsDir];
  process.env[LEGACY_JEV_ENV.secretsDir] = env.secretsDir;
  env.writeSecret('typesafe_api_key', 'legacy-directory-key\n');

  try {
    const config = resolveSystemOneProvider();
    assert.equal(config?.provider, 'typesafe');
    assert.equal(config?.apiKey, 'legacy-directory-key');
    assert.equal(config?.keyOrigin, '~/.pi/agent/secrets/typesafe_api_key');
  } finally {
    env.restore();
  }
});

test('test_provider_system_one_key_outranks_laya_key', () => {
  const env = isolateEnv({
    [SYSTEM_ONE_ENV.provider]: 'laya',
    [SYSTEM_ONE_ENV.apiKey]: 'generic-key',
    [SYSTEM_ONE_ENV.layaApiKey]: 'laya-key',
  });
  try {
    const config = resolveSystemOneProvider();
    assert.equal(config?.provider, 'laya');
    assert.equal(config?.apiKey, 'generic-key');
    assert.equal(config?.keyOrigin, '$PI_SYSTEM_ONE_API_KEY');
  } finally {
    env.restore();
  }
});

test('test_legacy_pi_jev_provider_overrides_work_and_canonical_values_win', () => {
  const legacy = isolateEnv({
    [LEGACY_JEV_ENV.provider]: 'openrouter',
    [LEGACY_JEV_ENV.apiKey]: 'legacy-key',
    [LEGACY_JEV_ENV.baseURL]: 'https://legacy.example.com',
    [LEGACY_JEV_ENV.model]: 'legacy-model',
  });
  try {
    const config = resolveSystemOneProvider();
  ...
  const both = isolateEnv({
    [LEGACY_JEV_ENV.provider]: 'typesafe',
    [LEGACY_JEV_ENV.apiKey]: 'legacy-key',
    [SYSTEM_ONE_ENV.provider]: 'openrouter',
    [SYSTEM_ONE_ENV.apiKey]: 'canonical-key',
  });
  try {
    const config = resolveSystemOneProvider();
    assert.equal(config?.provider, 'openrouter');
...
test('test_provider_auto_never_selects_laya', () => {
  const env = isolateEnv({ [SYSTEM_ONE_ENV.layaApiKey]: 'laya-key' });
  try {
    assert.equal(resolveSystemOneProvider(), null);
  } finally {
    env.restore();
  }
});
...
test('test_provider_laya_base_url_has_no_v1_suffix_and_accepts_overrides', () => {
  assert.equal(SYSTEM_ONE_PROVIDERS.laya.baseURL, 'http://127.0.0.1:8000');
  assert.equal(
    `${SYSTEM_ONE_PROVIDERS.laya.baseURL}/v1/systemone`,
    'http://127.0.0.1:8000/v1/systemone',
  );

  const env = isolateEnv({
    [SYSTEM_ONE_ENV.provider]: 'laya',
    [SYSTEM_ONE_ENV.baseURL]: 'http://127.0.0.1:18000',
    [SYSTEM_ONE_ENV.model]: 'multilingual',
  });
  try {
    const config = resolveSystemOneProvider();
...
test('test_laya_rejects_known_hosted_provider_base_urls', () => {
  for (const baseURL of ['https://api.typesafe.ai', 'https://openrouter.ai/api']) {
    const env = isolateEnv({
      [SYSTEM_ONE_ENV.provider]: 'laya',
      [SYSTEM_ONE_ENV.baseURL]: baseURL,
    });
    try {
      const config = resolveSystemOneProvider();
      assert.equal(config?.provider, 'laya');
...
test('test_provider_explicit_openrouter_uses_openrouter_base_url_and_key', () => {
  const env = isolateEnv({ OPENROUTER_API_KEY: 'or-key', [SYSTEM_ONE_ENV.provider]: 'openrouter' });
  try {
    const info = new SystemOneClient().getProviderInfo();
...
test('test_provider_forced_provider_wins_over_the_other_providers_key', () => {
  ...
  try {
    assert.equal(resolveSystemOneProvider()?.provider, 'openrouter');
  } finally {
    env.restore();
  }
});

test('test_provider_system_one_api_key_and_base_url_override_everything', () => {
  const env = isolateEnv({
    TYPESAFE_API_KEY: 'ts-key',
    [SYSTEM_ONE_ENV.apiKey]: 'custom-key',
    [SYSTEM_ONE_ENV.baseURL]: 'https://openrouter.ai/api',
    [SYSTEM_ONE_ENV.model]: 'typesafe/jev-1.13',
  });
  try {
    const config = resolveSystemOneProvider();
...
test('test_provider_openrouter_base_url_has_no_v1_suffix', () => {
  assert.equal(SYSTEM_ONE_PROVIDERS.openrouter.baseURL, 'https://openrouter.ai/api');
  assert.equal(SYSTEM_ONE_PROVIDERS.typesafe.baseURL, 'https://api.typesafe.ai');
  // The SDK appends the path itself; a /v1 suffix here would yield /api/v1/v1/systemone.
  assert.equal(
    `${SYSTEM_ONE_PROVIDERS.openrouter.baseURL}/v1/systemone`,
...
test('test_settings_selected_provider_honours_system_one_api_key', () => {
  const env = isolateEnv({ [SYSTEM_ONE_ENV.apiKey]: 'override-key' });
  try {
    const client = new SystemOneClient();
    client.setProviderOverrides({ provider: 'openrouter' });
    const info = client.getProviderInfo();
    assert.equal(info?.provider, 'openrouter');
    assert.equal(info?.keyOrigin, '$PI_SYSTEM_ONE_API_KEY');
...
test('test_system_one_provider_with_system_one_api_key_resolves_that_provider', () => {
  const env = isolateEnv({
    [SYSTEM_ONE_ENV.provider]: 'openrouter',
    [SYSTEM_ONE_ENV.apiKey]: 'override-key',
  });
  try {
    const info = new SystemOneClient().getProviderInfo();
    assert.equal(info?.provider, 'openrouter');
...
test('test_fallback_uses_system_one_api_key_for_the_other_provider', () => {
  const env = isolateEnv({ [SYSTEM_ONE_ENV.apiKey]: 'override-key' });
  try {
    ...
    assert.equal(fallback?.keyOrigin, '$PI_SYSTEM_ONE_API_KEY');
  } finally {
    env.restore();
  }
});
...
test('test_client_sends_string_state_unwrapped_and_labels_openrouter_requests', async () => {
  ...
  try {
    await new SystemOneClient().evaluate({ state: 'plain state', questions: NOUL_QUESTION });
...
test('test_score_answers_expose_probabilities_and_legend', async () => {
  ...
  try {
    const response = await new SystemOneClient().evaluate({
      state: 'state',
      questions: {
        severity: {
          type: 'score',
...
test('test_array_state_is_preserved_not_rewritten', async () => {
  ...
  try {
    await new SystemOneClient().evaluate({
      state: [1, 'two', { three: 3 }],
