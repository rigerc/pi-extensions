...
import { LEGACY_SESSION_ENTRY_TYPE, SESSION_ENTRY_TYPE } from '../src/config-store.js';
...
export interface SettingsHarness {
  ...
  legacyPathFor(scope: 'user' | 'project'): string;
...
export function makeSettingsHarness(options: HarnessOptions = {}): SettingsHarness {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'pi-system-one-settings-'));
  const userPath = path.join(dir, 'home', 'pi-system-one.json');
  const projectPath = path.join(dir, 'project', '.pi', 'pi-system-one.json');
  const legacyUserPath = path.join(dir, 'home', 'pi-jev.json');
  const legacyProjectPath = path.join(dir, 'project', '.pi', 'pi-jev.json');
  ...
  if (options.legacyUser) write(legacyUserPath, options.legacyUser);
  if (options.legacyProject) write(legacyProjectPath, options.legacyProject);
  ...
  const service = new SettingsService(host, systemOneClient, modes, {
    env: options.env ?? {},
    userPath,
    projectPath,
    legacyUserPath,
  ...
  return {
    ...
    legacyPathFor: (scope) => (scope === 'user' ? legacyUserPath : legacyProjectPath),
    sessionBranch: () => branch,
    cleanup: () => fs.rmSync(dir, { recursive: true, force: true }),
  };
}

/** Shape one `pi.appendEntry("pi-system-one-config", …)` entry as it appears in a session branch. */
...
export function legacyConfigEntry(data: Record<string, unknown>): Record<string, unknown> {
  return { type: 'custom', customType: LEGACY_SESSION_ENTRY_TYPE, data };
}
