...
test('AutoSystemOne stays off until enabled', async () => {
  ...
  const auto = new AutoSystemOne(systemOneClient, router, skillRouter, false);
...
test('AutoSystemOne skips when Jev is unconfigured', async () => {
  const { systemOneClient, router, skillRouter, calls } = stubs(false);
...
test('AutoSystemOne routes tools and skills in one Jev request', async () => {
  const { systemOneClient, router, skillRouter, calls, requests } = stubs();
  const auto = new AutoSystemOne(systemOneClient, router, skillRouter, true);
  ...
  assert.equal(
    'recent_context' in requests[0].state,
    true,
    'routers share prompt context with Jev',
  );
});

test('AutoSystemOne widens once when the coverage answer says the shortlist was incomplete', async () => {
  ...
  const { systemOneClient, router, skillRouter, calls, requests } = stubs(true, {
    sufficient: 0.1,
    tools,
  });
  // Tools only: the skill pool would otherwise need its own oversized fixture.
  const auto = new AutoSystemOne(systemOneClient, router, skillRouter, true, false);
  ...
  assert.equal(
    requests[0].state.tools.length,
    10,
    'the first pass is bounded by the candidate limit',
  );
  assert.deepEqual(
...
test('AutoSystemOne keeps first-pass results when the widening pass fails', async () => {
  let call = 0;
  const systemOneClient = {
    isConfigured: () => true,
    evaluate: async (request: any) => {
...
test('AutoSystemOne ignores slash commands and concurrent prompts, and never throws', async () => {
...
test('AutoSystemOne routes only tools when the skill path is off', async () => {
  const { systemOneClient, router, skillRouter, calls, requests } = stubs();
  const auto = new AutoSystemOne(systemOneClient, router, skillRouter, true, false);
...
test('AutoSystemOne routes only skills when the tool path is off', async () => {
  const { systemOneClient, router, skillRouter, calls, requests } = stubs();
  const auto = new AutoSystemOne(systemOneClient, router, skillRouter, false, true);
...
test('AutoSystemOne is disabled when both routing paths are off', async () => {
  const { systemOneClient, router, skillRouter, calls } = stubs();
...
test('AutoSystemOne paths can be toggled independently', async () => {
  const { systemOneClient, router, skillRouter, calls } = stubs();
  const auto = new AutoSystemOne(systemOneClient, router, skillRouter, false);
...
test('AutoSystemOne widens skills alone when only the skill shortlist was incomplete', async () => {
...
test('AutoSystemOne widening never enables a disabled path', async () => {
  const tools = Array.from({ length: 12 }, (_, i) => `tool_${i}`);
  const { systemOneClient, router, skillRouter, calls, requests } = stubs(true, {
    toolSufficient: 0.1,
    tools,
