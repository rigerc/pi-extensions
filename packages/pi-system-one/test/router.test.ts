...
test('ToolRouter shortlists inactive tools correctly using local keywords', () => {
  ...
  const router = new ToolRouter(mockPi, systemOneClient);
...
test('ToolRouter never offers its own System One tools as candidates', async () => {
  // Regression: after /system-one disable, routing used to re-activate system_one_find_skill/system_one_evaluate.
  const mockTools = [
    { name: 'read', description: 'Read files' },
...
test('ToolRouter accepts a probability exactly at the shared threshold and rejects below it', async () => {
  const withProbability = (value: number | undefined) => {
    ...
    return new ToolRouter(pi, systemOneClient);
  };

  const atCutoff = await withProbability(SYSTEM_ONE_THRESHOLD).findAndActivate('query sqlite');
  assert.deepEqual(atCutoff.activated, ['sqlite_query']);
  assert.equal(atCutoff.fallbackUsed, false);

  const belowCutoff = await withProbability(SYSTEM_ONE_THRESHOLD - 0.01).findAndActivate(
    'query sqlite',
  );
...
test('SystemOneClient handles unconfigured state safely without throwing in check', () => {
...
test('ToolRouter shortlist can exclude tools a previous pass already judged', () => {
  ...
  const router = new ToolRouter(pi, new SystemOneClient());
...
test('ToolRouter widens once when coverage says the shortlist was incomplete', async () => {
  ...
  assert.equal(
    activeTools.length,
    TOOL_CANDIDATE_LIMIT + 1,
    'activation stays additive across passes',
  );
});
