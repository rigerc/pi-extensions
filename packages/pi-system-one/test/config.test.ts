...
test('test_provenance_reports_the_winning_layer_per_key', () => {
  ...
  assert.equal(resolved.provenance.systemOneTools, 'default');
});
...
test('test_settings_from_env_reads_system_one_variables_and_skips_blanks', () => {
  const raw = settingsFromEnv({
    PI_SYSTEM_ONE_AUTO: '1',
    PI_SYSTEM_ONE_TOOL_GUARD: 'off',
...
test('test_legacy_env_variables_are_accepted_but_canonical_names_win', () => {
  const raw = settingsFromEnv({
    PI_JEV_AUTO: '1',
    PI_JEV_AUTO_SKILLS: '0',
    PI_JEV_PROVIDER: 'typesafe',
    PI_SYSTEM_ONE_PROVIDER: 'openrouter',
  });

  assert.equal(raw.autoToolRouting, '1');
  assert.equal(raw.autoSkillRouting, '0', 'specific legacy input beats the legacy master');
  assert.equal(raw.provider, 'openrouter', 'canonical env wins over its legacy alias');
  assert.deepEqual(
    effectiveLegacyInputs(
      { PI_JEV_AUTO: '1', PI_JEV_PROVIDER: 'typesafe', PI_SYSTEM_ONE_PROVIDER: 'openrouter' },
      () => undefined,
    ),
    ['$PI_JEV_AUTO'],
  );
...
test('test_a_specific_env_var_overrides_the_master_switch', () => {
  const raw = settingsFromEnv({ PI_SYSTEM_ONE_AUTO: '1', PI_SYSTEM_ONE_AUTO_SKILLS: '0' });
  assert.equal(raw.autoToolRouting, '1');
...
test('test_settings_from_flags_is_on_only_and_masters_expand', () => {
  ...
  const raw = settingsFromFlags((name) => {
    seen.push(name);
    return name === 'system-one-auto' ? true : name === 'system-one-compact' ? false : undefined;
  });
  ...
  assert.ok(seen.includes('system-one-tool-guard'), 'all boolean flags are queried');
});

test('test_a_specific_flag_turns_on_one_path_only', () => {
  const raw = settingsFromFlags((name) => (name === 'system-one-auto-tools' ? true : undefined));
  assert.deepEqual(raw, { autoToolRouting: true });
});

test('test_legacy_flags_are_accepted_and_reported', () => {
  const getFlag = (name: string) => (name === 'jev-tool-guard' ? true : undefined);
  assert.deepEqual(settingsFromFlags(getFlag), { toolGuard: true });
  assert.deepEqual(effectiveLegacyInputs({}, getFlag), ['--jev-tool-guard']);
});

test('test_noneditable_legacy_provider_env_is_reported_without_exposing_values', () => {
  ...
  assert.deepEqual(
    effectiveLegacyInputs(
      {
        PI_SYSTEM_ONE_API_KEY: 'canonical-secret',
        PI_JEV_API_KEY: 'legacy-secret',
      },
      () => undefined,
    ),
    [],
