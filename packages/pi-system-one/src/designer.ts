...
/** Upper bound on questions sent to System One per designed evaluation. */
...
/** Validate untrusted model output into a System One request. Returns null when unusable. */
export function validateDesign(raw: unknown): SystemOneEvaluationRequest | null {
  ...
  for (const [id, value] of Object.entries(candidate.questions as Record<string, unknown>)) {
    ...
    if (q.type === 'noul') {
      ...
      if (q.criteria && typeof q.criteria === 'object' && !Array.isArray(q.criteria)) {
        ...
        if (raw.false !== undefined) criteria.false = raw.false as SystemOneInstruction;
      }
      questions[id] =
...
/**
 * Ask the session's active model to design a System One evaluation for a free-form prompt.
...
export async function designEvaluation(
  ...
  prompt: string,
  signal?: AbortSignal,
): Promise<SystemOneEvaluationRequest> {
  ...
  if (!designed) {
    throw new Error(
      'Model did not return a usable System One question schema. Try rephrasing the prompt.',
    );
  }
  return designed;
}
