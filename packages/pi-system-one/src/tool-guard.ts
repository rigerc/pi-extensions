...
export class ToolGuard {
  ...
  public install(): void {
    this.pi.on('tool_call', async (event, ctx) => {
      ...
      if (isSystemOneTool(event.toolName)) return;

      // The deterministic path check runs with or without System One: it costs nothing and
      ...
      if (check.blocked) {
        ctx.ui.setStatus('system-one', `system-one: blocked hallucinated ${event.toolName}`);
        return {
          block: true,
          reason:
            check.reason ??
            'Blocked by System One tool-guard: hallucinated or invalid tool arguments detected.',
        };
      }
    });

    this.pi.on('tool_result', async (event, ctx) => {
      if (!this.enabled || !this.systemOneClient.isConfigured()) return;
      if (isSystemOneTool(event.toolName)) return;
      ...
      if (enhancement) {
        return {
          content: [
            ...
            {
              type: 'text' as const,
              text: `\n[System One Anti-Hallucination Guidance]: ${enhancement}`,
            },
          ],
        };
      }
  ...
  public async checkToolCall(
    ...
    if (!this.enabled || isSystemOneTool(toolName)) {
    ...
    if (missing.length > 0) {
      return {
        ...
        source: 'path-check',
        probability: 1,
        reason: `System One tool-guard blocked ${toolName}: ${missing
          .map((p) => `"${p}"`)
          .join(
            ', ',
          )} ${missing.length > 1 ? 'do not exist' : 'does not exist'} in ${cwd}. Verify with ls/find before guessing paths.`,
    ...
    try {
      ...
      return {
        ...
        reason: isHallucinated
          ? `System One tool-guard detected hallucinated parameters in ${toolName} call (P=${prob.toFixed(2)}). Check arguments against the tool schema.`
          : undefined,
        probability: prob,
        elapsedMs: Date.now() - startTime,
      };
  ...
  public async enhanceErrorResult(
    ...
    if (!this.enabled || !this.systemOneClient.isConfigured()) return null;
