...
/**
 * System One serves a 32K-token window, and a gate's evidence is a whole diff, file, or
...
export interface GateResult {
  passed: boolean;
  /** False when the gate passed without a System One judgment (fail-open on config/API error). */
  evaluated: boolean;
  ...
  threshold: number;
  elapsedMs: number;
  /** True when the evidence was cut to fit System One's context window. */
  truncated: boolean;
  answer?: SystemOneAnswerResult;
  error?: string;
}
...
Examples:
  subagent gate: "npx pi-system-one-gate -c 'Tests pass and no new any types' -d"
  pipeline gate: "git diff | npx pi-system-one-gate -c 'All exports documented'"
`);
}
...
export function collectGitEvidence(cwd = process.cwd()): GateEvidence {
  const run = (args: string[]): string => {
    ...
    if (result.status !== 0) {
      throw new Error(
        result.error?.message ?? result.stderr?.trim() ?? 'Git evidence collection failed',
      );
    }
  ...
  if (untracked.length > 0) {
    sections.push(
      `Untracked files (git diff omits these):\n${untracked.map((f) => `- ${f}`).join('\n')}`,
    );
  ...
  if (!client.isConfigured()) {
    if (options.failOpen) {
      ...
      return {
        ...
        elapsedMs: 0,
        truncated: false,
        error: 'System One unconfigured (fail-open enabled)',
      };
    }
