...
export function classifyModelError(error: unknown): ModelErrorKind {
  ...
  if (/context|too many tokens|token limit|maximum.*token|prompt too long/.test(text))
...
export class AutoModelRouter {
  ...
  public recordProviderResponse(status: number, model?: Model<any>): ModelErrorKind | undefined {
    ...
    if (['quota', 'rate-limit', 'context-limit', 'unavailable', 'timeout'].includes(kind)) {
      this.blocked.set(
        ...
        Date.now() + (kind === 'quota' || kind === 'rate-limit' ? 600_000 : 60_000),
      );
    }
    return kind;
  }
  ...
  public async route(
    ...
    const fallback: ModelRouteResult = {
      changed: false,
      profile: 'balanced',
      reason: 'model selection skipped',
    };
    ...
    try {
      ...
      const models = (
        ctx.scopedModels?.length
          ? ctx.scopedModels.map((x) => x.model)
          : ctx.modelRegistry.getAvailable()
      )
        .filter((model) => !options.hasImages || model.input?.includes('image'))
        .filter(
          (model) =>
            !this.blocked.get(`${model.provider}/${model.id}`) ||
            (this.blocked.get(`${model.provider}/${model.id}`) ?? 0) < Date.now(),
        );
      const target = models.sort(
        (a, b) => modelScore(b, need.profile) - modelScore(a, need.profile),
      )[0];
      if (!target)
        return {
          ...fallback,
      ...
      try {
        ...
        this.blocked.set(
          ...
          Date.now() + (kind === 'rate-limit' || kind === 'quota' ? 600_000 : 60_000),
        );
        return {
          changed: false,
          profile: need.profile,
          model: current,
          reason: `model switch failed: ${kind}`,
          skipped: 'error',
        };
      }
    } catch {
