...
export class ToolRouter {
  ...
  public async findAndActivate(
    query: string,
    threshold = SYSTEM_ONE_THRESHOLD,
    signal?: AbortSignal,
    ctx?: unknown,
    ...
    if (this.systemOneClient.isConfigured()) {
      try {
        const first = await this.judgePass(
          query,
          recentContext,
          firstCandidates,
          threshold,
        ...
        if (!first.sufficient) {
          ...
          if (remainder.length > 0) {
            try {
              const second = await this.judgePass(
                query,
                recentContext,
                remainder,
                threshold,
              ...
              activated = this.applyActivation(probabilities, threshold, [
                ...candidateNames,
                ...secondNames,
              ]);
              return {
    ...
    if (fallbackUsed) {
      // Fallback does not activate tools or claim certainty without System One judgment
