...
function fakeClient(configured = true): JevClient {
  return {
    getProviderInfo: () =>
      configured
        ? {
...
test("test_boolean_and_enum_rows_carry_cycling_values", () => {
  ...
  try {
    ...
    assert.equal(byId.get("autoSkillRouting")!.currentValue, "off", "the two paths are separate rows");
...
test("test_text_input_submenu_allows_empty_for_base_url", () => {
  let saved: string | undefined | "never" = "never";
  const submenu = new TextInputSubmenu("Base URL", "https://openrouter.ai/api", true, UI, (value) => {
    saved = value;
  });
