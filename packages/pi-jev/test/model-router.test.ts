...
for (const scoped of [false, true]) {
  test(`preserves the current model when ${scoped ? "scoped" : "available"} models cannot accept images`, async () => {
    ...
    let switches = 0;
    const router = new AutoModelRouter({ setModel: async () => { switches++; return true; } } as any, true);
    const ctx: any = {
      model: current,
      getSystemPrompt: () => "",
...
test("selects an image-capable model for image input", async () => {
  ...
  const router = new AutoModelRouter({ setModel: async (target: unknown) => { selected.push(target); return true; } } as any, true);
  const ctx: any = {
    model: current,
    getSystemPrompt: () => "",
...
test("does not fall back to a text-only model when the image model is blocked", async () => {
  ...
  const result = await router.route("inspect this attachment", {
    model: current,
    getSystemPrompt: () => "",
    modelRegistry: { getAvailable: () => [current, model("text-only")] },
  } as any, { hasImages: true });
  assert.equal(result.changed, false);
  assert.equal(result.skipped, "no-model");
  assert.equal(switches, 0);
});
