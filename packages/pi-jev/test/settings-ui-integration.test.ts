...
function openSettings() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-jev-ui-"));
  ...
  const settings = new SettingsService(host, jevClient, modes, {
  ...
  registerJevSettingsCommand(
    { registerCommand: (name: string, options: any) => { if (name === "jev-settings") handler = options.handler; } } as any,
    settings,
    jevClient
  );
  ...
  const ctx: any = {
    ui: {
      ...
      custom: async (factory: any) => {
        component = factory({ requestRender: () => { renders += 1; } }, { fg: (_c: string, t: string) => t, bold: (t: string) => t }, {}, () => { closed += 1; });
      },
    },
  };
  ...
  return {
    ...
    open: async () => {
      assert.ok(handler, "jev-settings must be registered");
      await handler!("", ctx);
...
test("test_jev_settings_command_registers_and_renders_every_group", async () => {
  const h = openSettings();
  try {
    const component = await h.open();
    const text = component.render(90).map(strip).join("\n");

    assert.match(text, /pi-jev settings/);
...
test("test_auto_tool_and_skill_rows_toggle_independently", async () => {
  ...
  try {
    ...
    assert.equal(h.auto.toolsEnabled, true, "live AutoJev tool path enabled");
    assert.equal(h.auto.skillsEnabled, false, "live AutoJev skill path still off");
...
test("test_jev_tools_row_updates_the_active_tool_set", async () => {
  const h = openSettings();
  try {
    const component = await h.open();
    for (let i = 0; i < ROW.jevTools; i++) component.handleInput(DOWN);
    component.handleInput(" ");

    assert.equal(h.settings.values.jevTools, true);
    assert.ok(h.activeTools().includes("jev_find_tools"));
