...
import { JEV_TOOL_NAMES } from "../src/types.js";
import { SESSION_ENTRY_TYPE } from "../src/config-store.js";
import { configEntry, makeSettingsHarness } from "./settings-harness.js";
...
test("test_init_resolves_files_env_flags_and_session_in_precedence_order", () => {
  const h = makeSettingsHarness({
    ...
    env: { PI_JEV_AUTO: "1", PI_JEV_PROVIDER: "openrouter" },
    branchEntries: [configEntry({ autoToolRouting: false, jevTools: true })],
  });
  h.setFlag("jev-compact", true);
  try {
    ...
    assert.equal(h.service.values.jevTools, true, "session entry only");
...
test("test_init_applies_settings_to_live_modes_tools_and_provider", () => {
  const h = makeSettingsHarness({
    user: { autoToolRouting: true, autoSkillRouting: true, autoModel: true, agentOrchestration: true, toolGuard: true, compaction: true, jevTools: true, provider: "openrouter", model: "typesafe/jev-1.13" },
  });
  try {
    h.service.init();
...
test("test_auto_paths_are_independent_and_a_session_override_beats_the_master_env_var", () => {
  // PI_JEV_AUTO is a master switch; a per-path session override must defeat it without
  // touching the other path.
  const h = makeSettingsHarness({ env: { PI_JEV_AUTO: "1" } });
  try {
    h.service.init();
...
test("test_jev_tools_grant_adds_and_removes_only_the_extension_tools", () => {
  ...
  try {
    h.service.init();
    h.service.set("jevTools", true);
    assert.deepEqual(
      h.activeTools().sort(),
      ["bash", "jev_evaluate", "jev_find_skill", "jev_find_tools", "read"].sort()
    );

    h.service.set("jevTools", false);
...
for (const scope of ["user", "project", "both"] as const) {
  test(`persisting to ${scope} merges overrides into each file without losing unrelated settings`, () => {
    ...
    const h = makeSettingsHarness({
      ...
      env: { PI_JEV_AUTO_MODEL: "1" },
    });
    try {
      h.service.init();
      h.service.set("toolGuard", false);
      const targets = scope === "both" ? ["user", "project"] as const : [scope];
      assert.deepEqual(h.service.persistToFile(scope), targets.map((target) => h.pathFor(target)));
...
test("persisting merges the latest file contents and excludes unknown secret fields", () => {
  ...
  try {
    ...
    fs.writeFileSync(h.pathFor("user"), JSON.stringify({
      version: 1,
      provider: "openrouter",
      model: "updated-model",
      apiKey: "test-secret",
...
test("test_reset_clears_session_overrides_and_optionally_deletes_files", () => {
  ...
  try {
    ...
    assert.equal(h.service.provenance.toolGuard, "project", "reloaded from disk, no longer session");
...
test("test_last_session_entry_wins_over_earlier_ones", () => {
  const h = makeSettingsHarness({
    branchEntries: [configEntry({ toolGuard: true, model: "first" }), configEntry({ model: "second" })],
  });
  try {
    h.service.init({ sessionManager: { getBranch: () => h.sessionBranch() } });
