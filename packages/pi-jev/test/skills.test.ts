...
test("SkillRouter normalizes Pi skill commands before recommending them", async () => {
  ...
  const router = new SkillRouter({
    getCommands: () => [
      { name: "skill:tdd", description: "Test-driven development", source: "skill", sourceInfo: { path: location } },
      { name: "jev", description: "Manage Jev", source: "extension" },
    ],
  } as any, { isConfigured: () => false } as JevClient);
...
test("SkillRouter deduplicates prompt skills and commands and preserves filePath", () => {
  const router = new SkillRouter({
    getCommands: () => [{ name: "skill:tdd", description: "Command description", source: "skill" }],
  } as any, { isConfigured: () => false } as JevClient);
  const skills = router.getAvailableSkills({
    getSystemPromptOptions: () => ({
      skills: [{ name: "tdd", description: "Test-driven development", filePath: "/skills/tdd/SKILL.md" }],
    }),
  } as any);
...
test("SkillRouter shortlist can exclude skills a previous pass already judged", () => {
  ...
  const router = new SkillRouter({ getCommands: () => [] } as any, new JevClient());
