...
function piWithTools(tools: Array<{ name: string; description?: string; parameters?: unknown }> = []) {
  return { on: () => {}, getAllTools: () => tools } as any;
}

test("ToolGuard skips when disabled or when the tool is one of our own", async () => {
  const guard = new ToolGuard(piWithTools(), new JevClient(), false);

  const res = await guard.checkToolCall("bash", { command: "ls -la" });
  assert.equal(res.valid, true);
  assert.equal(res.blocked, undefined);

  const enabled = new ToolGuard(piWithTools(), new JevClient(), true);
  assert.equal((await enabled.checkToolCall("jev_find_skill", { query: "x" })).valid, true);
});

test("ToolGuard blocks a path that does not exist without spending a Jev request", async () => {
  ...
  const guard = new ToolGuard(piWithTools(), mockJev, true);
  ...
  assert.equal(evaluated, 0, "a deterministic verdict must not cost a Jev request");
});

test("ToolGuard still blocks a missing path when Jev is unconfigured", async () => {
  const mockJev = { isConfigured: () => false } as unknown as JevClient;
  const guard = new ToolGuard(piWithTools(), mockJev, true);
...
test("ToolGuard does not path-check write, because creating a file is valid", async () => {
  ...
  const guard = new ToolGuard(piWithTools(), mockJev, true);
  const res = await guard.checkToolCall("write", { path: "src/brand-new-file.ts", content: "" }, ctx);
  assert.equal(res.valid, true);
  assert.equal(res.source, "jev", "write falls through to the shape check");
});
...
test("ToolGuard does not block a path a sibling write in the same batch is creating", async () => {
      ...
      getBranch: () => [
        {
          ...
          message: {
            role: "assistant",
            content: [
              { type: "toolCall", id: "1", name: "write", arguments: { path: "tmp/other-file.ts" } },
            ],
          },
        },
      ],
  ...
  assert.equal((await guard.checkToolCall("read", { path: "tmp/new-file.ts" }, otherCtx)).blocked, true);
});
...
test("ToolGuard path check ignores globs, URLs, and home-relative paths", () => {
  ...
  assert.deepEqual(findMissingPaths("read", { path: "~/.pi/agent/missing.json" }, process.cwd()), []);
  assert.deepEqual(findMissingPaths("bash", { path: "nope.ts" }, process.cwd()), [], "bash is not path-checked");
