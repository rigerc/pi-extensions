...
import { RECENT_CONTEXT_CHARS, lastAssistantText, lastAssistantToolCalls, recentContextFrom } from "../src/context.js";
...
test("lastAssistantText returns the tail of the newest assistant text", () => {
  const entries = [
    { type: "message", message: { role: "user", content: [{ type: "text", text: "do the same" }] } },
    {
      type: "message",
