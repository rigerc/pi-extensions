...
import { MAX_STATE_CHARS, MAX_STATE_FIELD_CHARS, capState } from "../src/jev.js";
...
test("capState shrinks the whole payload when the fields alone are still too large", () => {
  ...
  assert.equal((capped.value as Record<string, string>).criteria, "keep me", "small fields survive");
});
