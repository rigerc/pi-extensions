...
test("/jev status reports the active provider, session cost and cross-provider fallback", async () => {
  const { run, calls } = harness({}, {}, {
...
test("/jev auto on writes through to the settings layer and reports session provenance", async () => {
...
test("/jev auto-tools toggles only the tool path", async () => {
  const sh = makeSettingsHarness();
  try {
    sh.service.init();
    const { run, calls } = harness({}, {}, {}, sh.service);
...
test("/jev auto-skills toggles only the skill path and flips when given no argument", async () => {
  const sh = makeSettingsHarness();
  try {
    sh.service.init();
    const { run, calls } = harness({}, {}, {}, sh.service);
...
test("/jev disable writes through to the settings layer", async () => {
  const sh = makeSettingsHarness();
  try {
    sh.service.init();
    sh.service.set("jevTools", true);
    const { run, calls } = harness({}, {}, {}, sh.service);

    await run("disable");
    assert.equal(sh.service.values.jevTools, false);
    assert.deepEqual(sh.entries.at(-1)!.data, { jevTools: false });
    assert.match(calls.at(-1)!.message, /Jev tools disabled/);
  } finally {
    sh.cleanup();
  }
});
