...
import { capState, type JevClient } from "./jev.js";
import type { JevAnswerResult, NoulQuestionConfig } from "./types.js";
...
export class JevCompactor {
  ...
  public async compact(event: SessionBeforeCompactEvent, _ctx: ExtensionContext): Promise<CompactResult> {
    ...
    const incomplete = (): CompactResult => ({ summary: "", kept: 0, considered: entries.length, skipped: "incomplete" });
    ...
    try {
          ...
          questions[`keep_${item.index}`] = {
            ...
            criteria: {
              true: {
                ...
                examples: ["A failing test's exact error and file path", "A constraint the user stated"],
              },
              false: {
      ...
      const answers: Record<string, JevAnswerResult> = Object.keys(questions).length
        ? (await this.jevClient.evaluate({
            state,
            questions,
          }, event.signal)).answers
        : {};

      const kept: string[] = [];
      for (const item of candidates) {
        const probability = item.candidate
          ? answers[`keep_${item.index}`]?.value
          : 1;
