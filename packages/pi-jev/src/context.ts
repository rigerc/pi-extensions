/**
 * Conversation context shared with Jev's routing questions.
 *
 ...
 * candidate list out of Jev's 32K window.
 */
 ...
 */
export function lastAssistantToolCalls(
  sessionManager?: BranchReader | null
): AssistantToolCall[] {
  let branch: readonly unknown[] = [];
  try {
    branch = sessionManager?.getBranch?.() ?? [];
