...
export function registerJevTools(
  ...
  pi.registerTool({
    ...
    description:
      "Find and additively activate registered Pi tools needed for a task using Jev semantic evaluation.",
    ...
    async execute(_toolCallId, params: any, signal, onUpdate, ctx) {
      ...
      if (result.escalated) {
        summaryText += " (expanded: the first shortlist was judged incomplete, so a second pass considered more tools)";
      }
  ...
  pi.registerTool({
    ...
    description:
      "Find and recommend the best matching agent skills for a specific task or problem using Jev semantic evaluation.",
  ...
  pi.registerTool({
    ...
    label: "Jev Evaluate",
    description:
      "Ask Jev System One typed questions (choice, noul, score) about structured state. Returns calibrated probabilities.",
    ...
    async execute(_toolCallId, params: any, signal, onUpdate) {
      ...
      onUpdate?.({
        content: [{ type: "text", text: "Querying Jev model..." }],
        details: {},
      });
