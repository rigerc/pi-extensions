...
export class TestConnectivitySubmenu {
  ...
  render(width: number): string[] {
    return [truncateToWidth(this.ui.bold("Actions · Test connectivity"), width), ...this.lines.map((l) => truncateToWidth(l, width))];
  }
...
export function buildSettingItems(
  ...
  for (const spec of SETTING_SPECS) {
    ...
    items.push({
      ...shared,
      currentValue: formatSettingValue(value) === "(provider default)" ? "(provider default)" : String(value),
      submenu: (_current, done) =>
        new TextInputSubmenu(label, String(value), Boolean(spec.allowEmpty), ui, done),
    });
  }

  const provider = jevClient.getProviderInfo();
  ...
  items.push({
    ...
    currentValue: `${jevClient.stats.requestsCount} req`,
    submenu: (_current, done) =>
      new InfoSubmenu(
        ui.accent(ui.bold("Status · Session")),
        [
          `  configured:  ${jevClient.isConfigured() ? "yes" : "no"}`,
          `  provider:    ${provider?.provider ?? "—"}`,
          `  model:       ${provider?.model ?? "—"}`,
          `  requests:    ${jevClient.stats.requestsCount}`,
          `  tokens:      ${jevClient.stats.totalTokens}`,
          `  cost:        ${jevClient.stats.totalCostUsd > 0 ? `$${jevClient.stats.totalCostUsd.toFixed(6)}` : "n/a"}`,
          `  fallback:    ${jevClient.stats.fallback ? `${jevClient.stats.fallback.from} → ${jevClient.stats.fallback.to}` : "none"}`,
          "",
  ...
  items.push({
    ...
    currentValue: "run",
    submenu: (_current, done) => new TestConnectivitySubmenu(jevClient, ui, done, requestRender),
  });
  ...
  items.push({
    ...
    submenu: (_current, done) =>
      new ChoiceSubmenu(
        "Reset to defaults",
        [
          { value: "session", label: "Clear session overrides", description: "keeps settings files" },
          { value: "files", label: "Clear everything", description: "also deletes both settings files" },
        ],
        ui,
...
/** Register `/jev-settings`, opening the editor as an overlay. */
export function registerJevSettingsCommand(
  pi: ExtensionAPI,
  settings: SettingsService,
  jevClient: JevClient
): void {
  pi.registerCommand("jev-settings", {
    description: "Open the interactive pi-jev settings editor",
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ctx.ui.custom<void>(
        (tui, theme, _keybindings, done) => {
          ...
          container.addChild(new Text(ui.accent(ui.bold("pi-jev settings")), 1, 0));
          container.addChild(
            new Text(
              ui.dim("Edits apply immediately and persist for this session. Use Persist to file to keep them."),
              1,
              0
            )
          );

          const items = buildSettingItems(settings, jevClient, ui, () => tui.requestRender());
          const list = new SettingsList(
            ...
            (id, newValue) => {
              if (isSettingKey(id) && !settings.set(id, newValue)) {
                ctx.ui.notify(`pi-jev settings: rejected value "${newValue}" for ${id}`, "warning");
              }
              tui.requestRender();
            },
            () => done(),
